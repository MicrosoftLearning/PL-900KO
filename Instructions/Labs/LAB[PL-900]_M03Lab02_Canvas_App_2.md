...
2.  새 캔버스 애플리케이션 만들기
    ...
    -   **새로 만들기**를 클릭하고 **앱 \| 캔버스 앱 \| 전화 폼 팩터** 를 선택합니다.
    ...
    -   **클라우드**가 선택되어 있는지 확인합니다. 이름에 대한 **캠퍼스 보안**을 입력하고
        **저장** 을 클릭합니다. 이렇게 하면 앱이 예기치 않게 닫혔을 때 변경 내용을 잃지 않습니다.
    -   앱으로 돌아가기 위해 오른쪽 상단("Power Apps" 아래)의 뒤로 화살표를 클릭합니다.

3.  데이터 원본에 연결(방문)
    1.  **보기 \| 데이터 원본** 을 클릭하세요.
    2.  **데이터 추가**를 클릭하세요.
    3.  **모든 엔터티 보기**를 클릭합니다.
    4.  **방문**을 선택하고 방문 엔터티가 데이터 원본 아래에 표시될 때까지 기다립니다.
4.  진행 중인 작업을 유지하려면 **File \**을 클릭합니다**| 저장**을 클릭한 다음 **저장**을 누릅니다.
...
2. 양식 보기 추가
   ...
   -   고급 탭에서 **항목** 속성을 선택하고 `LookUp(Visits, Code = textCode.Text)`를 입력합니다. 

3.  진행 중인 작업을 유지하려면 **File \**을 클릭합니다**| 저장**을 누른 다음에 **저장**을 누릅니다. 앱으로 돌아가기 위해 오른쪽 상단("Power Apps" 아래)의 뒤로 화살표를 클릭합니다.
...
4. 앱 테스트
   ...
   -   **방문** 엔터티를 선택합니다.
   -   **데이터** 탭을 선택합니다.
   -   현재 뷰 이름(활성 방문)을 클릭하여 오른쪽 상단에 있는 뷰 선택을 엽니다.
   -   뷰를 **모든 필드**로 변경합니다.
...
1. **삽입** 탭을 선택합니다.
   2. **단추**를 클릭합니다.
   3. 단추 **텍스트** 속성을 **"체크 인"**으로 변경합니다(기존 따옴표 내에서 입력할 수 있음)
   4. 오른쪽 패널의 단추 이름(단추 1)을 클릭하여 단추 이름을 **CheckInButton**으로 변경합니다.
...
3. 방문 데이터에 따라 단추를 활성화 및 비활성화합니다. 
   ...
   * 체크 인 단추를 선택하고 속성 탭에 있는 단추의 **DisplayMode** 속성을 클릭합니다.
   ...
   * `&&` - 논리 AND 연산자
   * `Visit.Status = 'Status (Visits)'.Active` - 레코드의 상태가 *활성*임
...
4. 방문 레코드가 있고(비어 있지 않음) 레코드 상태가 활성화되어 있으며 방문이 이미 시작된 경우(즉, 실제 시작 값이 비어 있지 않음) **체크 아웃** 단추를 사용하도록 설정합니다.
   ...
   * 체크 아웃 단추를 선택하고 속성 탭에 있는 단추의 **DisplayMode** 속성을 클릭합니다.
...
5. 진행 중인 작업을 유지하려면 **File \**을 클릭합니다**| 저장**을 누른 다음에 **저장**을 누릅니다. 앱으로 돌아가기 위해 오른쪽 상단("Power Apps" 아래)의 뒤로 화살표를 클릭합니다.
...
체크인 및 체크아웃 프로세스를 수행하려면 다음과 같이 Common Data Service 방문 데이터를 업데이트해야 합니다.

* 방문자가 체크인 할 때 *실제 시작* 필드를 현재 날짜 및 시간으로 설정합니다.
* 방문자가 체크 아웃할 때 *실제 종료* 필드를 현재 날짜 및 시간으로 설정합니다. 
* 체크아웃 후 레코드 상태를 방문이 완료되었음을 나타내는 비활성 상태로 설정합니다.
...
5. 진행 중인 작업을 유지하려면 **File \**을 클릭합니다**| 저장**을 누른 다음에 **저장**을 누릅니다.
...
2. **아이콘 \| 추가를** 선택합니다. 이 시점에서는 값이 동적이기를 원하기 때문에 어떤 아이콘을 선택하는지는 중요하지 않습니다.

3. 단추 아래의 화면 중앙에 있는 아이콘 크기 다시 조정 및 배치

4. 아이콘의 고급 탭에서 **아이콘**속성(디자인 섹션에서)을 선택하고 다음 식을 입력
...
7. 이전에 사용되지 않은 다른 코드 값을 찾습니다(실제 시작 또는 실제 끝 값이 없어야 함). 이제 이전에 만든 **Campus Staff** 앱을 실행하여 새 방문 레코드를 만들 수 있습니다. 아이콘이 미소 이모지를 표시했는지 확인합니다.
8. **ESC** 키를 눌러 실행 중인 앱을 종료합니다.

## 작업 #6: 앱을 게시합니다.

1. 브라우저에서 캠퍼스 보안 앱을 열어야 합니다. 그렇지 않은 경우, **캠퍼스 보안** 앱을 선택하고 **편집**을 클릭합니다.
2. **파일 \| 게시**를 선택합니다.
3. **이 버전 게시**를 선택합니다.
