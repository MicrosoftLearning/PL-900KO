...
2.  게시자 만들기
    ...
    -   표시되는 창에서 **표시 이름**으로 **벨로즈 대학**을 입력하고 **접두사**로 **bc**를 입력합니다.
...
3. **상태가** **꺼짐**인 경우 **데이터 가져오기** 옆의 **...** 를 선택하고 **켜짐**을 선택합니다.

   > **중요:** 오류 메시지를 수신한 경우, 만들어진 엔터티 및 필드가 위의 지침과 일치하는지 확인합니다.

4. **데이터 가져오기** 구성 요소를 선택합니다. 새 탭에서 Power Automate가 열립니다. 팝업이 나타나는 경우 **시작하기**를 클릭합니다. 

5. **실행**을 클릭한 다음 메시지가 표시되면 **흐름 실행**을 클릭합니다.

6. **완료**를 클릭합니다.

7. 흐름 인스턴스가 실행을 완료할 때까지 기다립니다. **28일간의 실행 기록** 테이블을 새로 고쳐 흐름이 실행된 시기를 확인할 수 있습니다. 이 흐름의 목적은 이후의 랩에 대한 예제 데이터를 생성하는 것이었습니다. 다음 작업에서는 데이터 가져오기가 성공했는지 확인합니다. 
...
4. **건물** 열에서 값을 클릭하고 건물 양식이 별도 창에서 열리는지 확인합니다. 이 창을 닫습니다.

5. **방문자** 열에서 값을 클릭하고(뷰를 오른쪽으로 스크롤해야 할 수도 있음) 연락처 양식이 별도 창에서 열리는지 확인합니다. 이 창을 닫습니다.
